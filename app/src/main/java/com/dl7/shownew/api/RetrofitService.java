...
import com.dl7.shownew.api.bean.PhotoInfo;
...
import com.dl7.shownew.api.bean.WelfarePhotoList;
import com.dl7.shownew.local.table.BeautyPhotoInfo;
import com.dl7.shownew.local.table.VideoInfo;
import com.dl7.shownew.utils.NetUtil;
import com.dl7.shownew.utils.StringUtils;
...
public class RetrofitService {
  ...
  private static final Interceptor sRewriteCacheControlInterceptor = new Interceptor() {
    ...
    @Override public Response intercept(Chain chain) throws IOException {
      ...
      if (NetUtil.isNetworkAvailable(AndroidApplication.getContext())) {
        ...
        Logger.e("no network cacheControl: " + cacheControl);
  ...
  private static final Interceptor sLoggingInterceptor = new Interceptor() {
    ...
    @Override public Response intercept(Chain chain) throws IOException {
      ...
      //打印url信息
      Logger.w(request.url() + (request.body() != null ? "?" + _parseParams(request.body(),
          requestBuffer) : ""));

      //Logger.e("wyl", "url " + request.url() + " method " + request.method() + "  body " + request.body().toString());
  ...
  @NonNull private static String _parseParams(RequestBody body, Buffer requestBuffer)
   ...
   */
  @Deprecated public static class FlatMapTransformer<T>
      implements Observable.Transformer<Map<String, List<T>>, T> {
    ...
    @Override public Observable<T> call(Observable<Map<String, List<T>>> mapObservable) {
      return mapObservable.flatMap(new Func1<Map<String, List<T>>, Observable<T>>() {
        @Override public Observable<T> call(Map<String, List<T>> stringListMap) {
   ...
   * @param typeStr 新闻类型
   */
  private static Func1<Map<String, List<NewsInfo>>, Observable<NewsInfo>> _flatMapNews(
      final String typeStr) {
    return new Func1<Map<String, List<NewsInfo>>, Observable<NewsInfo>>() {
      @Override public Observable<NewsInfo> call(Map<String, List<NewsInfo>> newsListMap) {
   ...
   * @param typeStr 视频类型
   */
  private static Func1<Map<String, List<VideoInfo>>, Observable<List<VideoInfo>>> _flatMapVideo(
      final String typeStr) {
    return new Func1<Map<String, List<VideoInfo>>, Observable<List<VideoInfo>>>() {
      @Override public Observable<List<VideoInfo>> call(Map<String, List<VideoInfo>> newsListMap) {
   ...
   */
  private static Func1<Map<String, SpecialInfo>, Observable<SpecialInfo>> _flatMapSpecial(
      final String specialId) {
    return new Func1<Map<String, SpecialInfo>, Observable<SpecialInfo>>() {
      @Override public Observable<SpecialInfo> call(Map<String, SpecialInfo> specialMap) {
  ...
  private static Func1<Map<String, List<BeautyPhotoInfo>>, Observable<List<BeautyPhotoInfo>>> _flatMapPhotos() {
    return new Func1<Map<String, List<BeautyPhotoInfo>>, Observable<List<BeautyPhotoInfo>>>() {
      @Override public Observable<List<BeautyPhotoInfo>> call(
          Map<String, List<BeautyPhotoInfo>> newsListMap) {
  ...
  private static Func1<WelfarePhotoList, Observable<WelfarePhotoInfo>> _flatMapWelfarePhotos() {
    return new Func1<WelfarePhotoList, Observable<WelfarePhotoInfo>>() {
      @Override public Observable<WelfarePhotoInfo> call(WelfarePhotoList welfarePhotoList) {
   ...
   * @throws IOException
   */
  public String getSignature(HashMap<String, String> params, String secret) {
    // 先将参数以其参数名的字典序升序进行排序
